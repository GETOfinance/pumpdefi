...
const ADDR = { BORROW_MANAGER: process.env.BORROW_MANAGER_ADDRESS }

router.get('/campaign/:id', async (req, res) => {
  try {
    const id = BigInt(req.params.id)
    if (!ADDR.BORROW_MANAGER) return res.status(503).json({ success: false, error: 'No borrow contract' })
    const mgr = new ethers.Contract(ADDR.BORROW_MANAGER, BORROW_MANAGER_ABI, provider)
    const info = await mgr.getCampaign(id)
    res.json({ success: true, info })
  } catch (e) {
    console.error('borrow campaign', e)
