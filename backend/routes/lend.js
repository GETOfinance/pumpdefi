...
const ADDR = { LEND_MANAGER: process.env.LEND_MANAGER_ADDRESS }

router.get('/campaign/:id', async (req, res) => {
  try {
    const id = BigInt(req.params.id)
    if (!ADDR.LEND_MANAGER) return res.status(503).json({ success: false, error: 'No lend contract' })
    const mgr = new ethers.Contract(ADDR.LEND_MANAGER, LEND_MANAGER_ABI, provider)
    const info = await mgr.getCampaign(id)
    res.json({ success: true, info })
  } catch (e) {
    console.error('lend campaign', e)
