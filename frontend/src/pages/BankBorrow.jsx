...
import { BORROW_MANAGER_ABI, ERC20_ABI } from '../config/abis'
...
const BankBorrow = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const mgr = getContract({ address: CONTRACT_ADDRESSES.BORROW_MANAGER, abi: BORROW_MANAGER_ABI, publicClient })
  ...
  const createCampaign = async () => {
    try {
      ...
      if (!isAddress(collateralToken) || !isAddress(rewardToken)) { toast.error('Invalid token'); return }
      const mgrWrite = getContract({ address: CONTRACT_ADDRESSES.BORROW_MANAGER, abi: BORROW_MANAGER_ABI, walletClient })
      const mgrRead = getContract({ address: CONTRACT_ADDRESSES.BORROW_MANAGER, abi: BORROW_MANAGER_ABI, publicClient })
      ...
      const id = await mgrWrite.write.createCampaign([collateralToken, rewardToken, amt, BigInt(period)], { value: fee })
  ...
  const fundCampaign = async () => {
    try {
      ...
      const mgr = getContract({ address: CONTRACT_ADDRESSES.BORROW_MANAGER, abi: BORROW_MANAGER_ABI, walletClient })
      ...
      await erc.write.approve([CONTRACT_ADDRESSES.BORROW_MANAGER, amt])
  ...
  const doProvide = async () => {
    try {
      ...
      const mgr = getContract({ address: CONTRACT_ADDRESSES.BORROW_MANAGER, abi: BORROW_MANAGER_ABI, walletClient })
      const info = await mgr.read.getCampaign([BigInt(campaignId)])
      const dec = await (getContract({ address: info.collateralToken, abi: ERC20_ABI, publicClient }).read.decimals())
      const amt = parseUnits(collateralAmount, Number(dec))
      const erc = getContract({ address: info.collateralToken, abi: ERC20_ABI, walletClient })
      await erc.write.approve([CONTRACT_ADDRESSES.BORROW_MANAGER, amt])
      await mgr.write.provideCollateral([BigInt(campaignId), amt, BigInt(lockSeconds || '0')])
      toast.success('Provided Collateral')
    } catch (e) {
      console.error(e)
      toast.error(e?.shortMessage || e?.message || 'Provide failed')
    }
  }
  ...
  const doWithdraw = async () => {
    try {
      ...
      const dec = await (getContract({ address: info.collateralToken, abi: ERC20_ABI, publicClient }).read.decimals())
      const amt = parseUnits(collateralAmount, Number(dec))
      await mgr.write.withdrawCollateral([BigInt(campaignId), amt])
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      <h1 className="text-3xl font-bold gradient-text">Borrow (Collateral Incentives)</h1>

      <div className="card p-6 space-y-3">
        <h2 className="text-xl font-semibold">Create Campaign</h2>
        <div className="grid md:grid-cols-2 gap-3">
          <input className="input" placeholder="Collateral token address" value={collateralToken} onChange={e=>setCollateralToken(e.target.value)} />
      ...
      <div className="card p-6 space-y-3">
        <h2 className="text-xl font-semibold">Provide / Claim / Withdraw</h2>
        <div className="grid md:grid-cols-4 gap-3">
          <input className="input" placeholder="Campaign ID" value={campaignId} onChange={e=>setCampaignId(e.target.value)} />
          <input className="input" placeholder="Collateral amount (token units)" value={collateralAmount} onChange={e=>setCollateralAmount(e.target.value)} />
          <input className="input" placeholder="Lock seconds (optional)" value={lockSeconds} onChange={e=>setLockSeconds(e.target.value)} />
          <button className="btn btn-primary" onClick={doProvide}>Provide</button>
        </div>
      ...
      <div className="text-xs text-gray-500">Note: Rewards stream continuously based on an hourly or daily rate; creator must keep the contract funded. Locking prevents withdrawal until the set time.</div>
    </div>
  )
}

export default BankBorrow
